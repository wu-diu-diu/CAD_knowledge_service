...
class WiringPolicyNet(nn.Module):
    ...
    def forward(
        ...
        # 直接从 obs 空间归一化到 [-1,1]（align_corners=True 语义）
        # obs 坐标 [0, H-1] → [-1, 1]，feat_map 分辨率差异由 grid_sample 内部处理
        norm_x = (lamp_c / (W - 1)) * 2.0 - 1.0  # col → x
        norm_y = (lamp_r / (H - 1)) * 2.0 - 1.0  # row → y
