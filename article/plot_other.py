...
\draw [-stealth, line width=1.5pt, draw=\edgecolor]
    (""" + from_node + r"""-east)
    -- node[""" + label_pos + r""", font=\normalsize\bfseries, text=black] {""" + label + r"""} (""" + to_node + r"""-west);
"""
...
def to_title(left_anchor: str, right_anchor: str, text: str, yshift: str = "2.6cm") -> str:
    return r"""
\node[font=\bfseries\LARGE, text=black]
...
def build_convblock_arch() -> list[str]:
    ...
    arch.extend(
        [
            ...
            to_arrow_with_label("cb_in", "cb_mid", "Conv2d(3x3)+GroupNorm+ReLU"),
            ...
            to_arrow_with_label("cb_mid", "cb_out", "Conv2d(3x3)+GroupNorm+ReLU"),
            to_title("cb_in-west", "cb_out-east", "ConvBlock"),
            to_end(),
        ]
    )
    return arch
...
def build_upblock_arch() -> list[str]:
    ...
    arch.extend(
        [
            ...
            to_arrow_with_label("up_in", "up_out", r"ConvTranspose2d(k=2,s=2)"),
            ...
            to_arrow_with_label("up_out", "cat_out", "Concat"),
            ...
            to_arrow_with_label("cat_out", "fuse_out", "Fuse"),
            to_title("up_in-west", "fuse_out-east", "UpBlock"),
            to_end(),
        ]
    )
    return arch
...
def build_fuse_arch() -> list[str]:
    ...
    arch.extend(
        [
            ...
            to_arrow_with_label("fuse_in", "fuse_only_out", "Conv2d(3x3)+GroupNorm+ReLU"),
            to_title("fuse_in-west", "fuse_only_out-east", "Fuse"),
            to_end(),
        ]
    )
    return arch
...
def build_stop_head_arch() -> list[str]:
    ...
    arch.extend(
        [
            ...
            to_tensor_block("sh_pool", "(3,0,0)", "(sh_in-east)", width=2, height=10, depth=10, fill=r"\FeatureColor"),
            to_label_below("sh_pool", r"$256 \times 1 \times 1$", xshift="-0.5cm", yshift="0.9cm"),
            to_arrow_with_label("sh_in", "sh_pool", r"AvgPool2d"),
            to_tensor_block("sh_flat", "(3,0,0)", "(sh_pool-east)", width=3, height=8, depth=1, fill=r"\VectorColor"),
            to_label_below("sh_flat", r"$1 \times 256$", xshift="-0.4cm", yshift="0.7cm"),
            to_arrow_with_label("sh_pool", "sh_flat", "Flatten"),
            to_tensor_block("sh_hidden", "(3,0,0)", "(sh_flat-east)", width=2, height=10, depth=1, fill=r"\VectorColor"),
            to_label_below("sh_hidden", r"$128$", xshift="-0.4cm", yshift="0.7cm"),
            to_arrow_with_label("sh_flat", "sh_hidden", r"Linear+ReLU"),
            to_tensor_block("sh_out", "(3,0,0)", "(sh_hidden-east)", width=1, height=5, depth=1, fill=r"\VectorColor"),
            to_label_below("sh_out", r"$1$", xshift="-0.3cm", yshift="0.6cm"),
            to_arrow_with_label("sh_hidden", "sh_out", r"Linear"),
